...
let ast ~(scheme : Scheme.t) ~(async : bool) ~(metadata : unit option) ~loc =
  ...
  let base =
    ...
    ; "submit", Uncurried.fn ~loc ~arity:1 [%expr fun () -> dispatch Submit]
    ; ( "mapSubmissionError"
      , Uncurried.fn ~loc ~arity:1 [%expr fun map -> dispatch (MapSubmissionError map)] )
    ; ( "dismissSubmissionError"
      , Uncurried.fn ~loc ~arity:1 [%expr fun () -> dispatch DismissSubmissionError] )
    ; ( "dismissSubmissionResult"
      , Uncurried.fn ~loc ~arity:1 [%expr fun () -> dispatch DismissSubmissionResult] )
    ; "reset", Uncurried.fn ~loc ~arity:1 [%expr fun () -> dispatch Reset]
    ]
  in
  let update_fns =
    scheme
                          ...
                          Exp.construct
                            ...
                            (Some
                               (Uncurried.fn
                                  ~loc
                                  ~arity:1
                                  [%expr fun __x -> nextInputFn __x nextValue]))]] )
              :: acc
